...
{$ENDIF}
  ...
  // /////////////////////////////////////////////////////////////////////////////////////////////////////////////////
  // TncLine
  // Bring in all functionality from WinSock API, with error handling via return codes instead of exceptions
...
{$ENDIF}
    ...
    function SetReceiveTimeout(const Value: Integer): Boolean;
    function GetSendTimeout: Integer;
    function SetSendTimeout(const Value: Integer): Boolean;
    ...
    function SetError(const AError: string; AErrorCode: Integer = 0): Boolean;
  protected const
    DefaultKind = stTCP;
  ...
  protected
    ...
    function CheckResult(aCmdRes: Integer): Boolean; inline;

    // API functions - now return Boolean for success/failure
    function CreateClientHandle(const aHost: string; const aPort: Integer;
      const aBroadcast: Boolean = False): Boolean;
    function CreateServerHandle(const aPort: Integer): Boolean;
    ...
    function EnableNoDelay: Boolean; inline;
    function EnableKeepAlive: Boolean; inline;
    function EnableBroadcast: Boolean; inline;
    function EnableIPv6Only: Boolean; inline;

    function EnableReuseAddress: Boolean; inline;
    function SetReceiveSize(const aBufferSize: Integer): Boolean;
    function SetWriteSize(const aBufferSize: Integer): Boolean;
  ...
  public
    ...
    property SendTimeout: Integer read GetSendTimeout;

    // Error handling properties instead of exceptions
...
begin
  ...
  if iRes <> 0 then
    raise Exception.Create(SysErrorMessage(iRes));
end;
...
function TncLine.CheckResult(aCmdRes: Integer): Boolean;
begin
  Result := True;
...
{$IFDEF MSWINDOWS}
    ...
    FLastError := SysErrorMessage(FLastErrorCode);
{$ELSE}
...
function TncLine.SetError(const AError: string; AErrorCode: Integer = 0): Boolean;
begin
  FLastError := AError;
...
function TncLine.CreateClientHandle(const aHost: string; const aPort: Integer;
  const aBroadcast: Boolean = False): Boolean;
var
{$IFDEF MSWINDOWS}
  Hints: TAddrInfoW;
...
begin
  ...
  try
    ...
    begin
      ...
      begin
        SetError('Invalid IPv6 address format: ' + aHost);
        Exit;
      end;
    end;
    ...
    begin
      SetError('Cannot use broadcast address when Broadcast is False');
      Exit;
    end;
...
{$ENDIF}
    try
      ...
      if not CheckResult(FHandle) then
        Exit;
      try
{$IFNDEF MSWINDOWS}
        if not EnableReuseAddress then
          Exit;
{$ENDIF}
        ...
        begin
          ...
          if not CheckResult(ConnectResult) then
            Exit;
          SetConnected;
        end
        else
...
function TncLine.CreateServerHandle(const aPort: Integer): Boolean;
var
{$IFDEF MSWINDOWS}
  Hints: TAddrInfoW;
{$ELSE}
...
{$ENDIF}
      // Bind the socket
      if not CheckResult(bind(FHandle, AddrResult^.ai_addr^, AddrResult^.ai_addrlen)) then
        Exit;

      // For TCP, we need to listen for incoming connections
      if IsConnectionBased then
        if not CheckResult(Listen(FHandle, SOMAXCONN)) then
          Exit;
...
begin
  ...
  begin
    ...
    begin
      ...
      CheckResult(Result);  // UDP: normal error check
  end
  else
...
begin
  ...
  begin
    ...
    begin
      DestroyHandle;
      Result := -1; // Indicate error/disconnection
    end
    else
      CheckResult(Result);  // UDP: normal error check
  end
...
function TncLine.EnableNoDelay: Boolean;
var
  optval: Integer;
begin
  optval := 1;
{$IFDEF MSWINDOWS}
  Result := CheckResult(SetSockOpt(FHandle, IPPROTO_TCP, TCP_NODELAY, PAnsiChar(@optval),
    SizeOf(optval)));
{$ELSE}
  Result := CheckResult(SetSockOpt(FHandle, IPPROTO_TCP, TCP_NODELAY, optval, SizeOf(optval)));
{$ENDIF}
end;

function TncLine.EnableKeepAlive: Boolean;
var
  optval: Integer;
begin
  optval := 1; // any non zero indicates true
{$IFDEF MSWINDOWS}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_KEEPALIVE, PAnsiChar(@optval),
    SizeOf(optval)));
{$ELSE}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_KEEPALIVE, optval, SizeOf(optval)));
{$ENDIF}
end;

function TncLine.EnableBroadcast: Boolean;
var
  optval: Integer;
begin
  optval := 1;
{$IFDEF MSWINDOWS}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_BROADCAST, PAnsiChar(@optval),
    SizeOf(optval)));
{$ELSE}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_BROADCAST, optval, SizeOf(optval)));
{$ENDIF}
end;

function TncLine.EnableIPv6Only: Boolean;
var
  optval: Integer;
begin
  ...
  begin
    optval := 1;
{$IFDEF MSWINDOWS}
    Result := CheckResult(SetSockOpt(FHandle, IPPROTO_IPV6, IPV6_V6ONLY, PAnsiChar(@optval),
      SizeOf(optval)));
{$ELSE}
    Result := CheckResult(SetSockOpt(FHandle, IPPROTO_IPV6, IPV6_V6ONLY, optval,
      SizeOf(optval)));
{$ENDIF}
  end
  else
...
function TncLine.EnableReuseAddress: Boolean;
var
  optval: Integer;
begin
  optval := 1;
{$IFDEF MSWINDOWS}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_REUSEADDR, PAnsiChar(@optval),
    SizeOf(optval)));
{$ELSE}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_REUSEADDR, optval, SizeOf(optval)));
{$ENDIF}
end;
...
function TncLine.SetReceiveSize(const aBufferSize: Integer): Boolean;
begin
  // min is 512 bytes, max is 1048576
{$IFDEF MSWINDOWS}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_RCVBUF, PAnsiChar(@aBufferSize),
    SizeOf(aBufferSize)));
{$ELSE}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_RCVBUF, aBufferSize,
    SizeOf(aBufferSize)));
{$ENDIF}
end;

function TncLine.SetWriteSize(const aBufferSize: Integer): Boolean;
begin
{$IFDEF MSWINDOWS}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_SNDBUF, PAnsiChar(@aBufferSize),
    SizeOf(aBufferSize)));
{$ELSE}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_RCVBUF, aBufferSize,
    SizeOf(aBufferSize)));
...
begin
  OptSize := SizeOf(Opt);
{$IFDEF MSWINDOWS}
  SocketResult := GetSockOpt(FHandle, SOL_SOCKET, SO_RCVTIMEO, PAnsiChar(@Opt), OptSize);
{$ELSE}
  SocketResult := GetSockOpt(FHandle, SOL_SOCKET, SO_RCVTIMEO, Opt, OptSize);
{$ENDIF}
  if CheckResult(SocketResult) then
    Result := Opt
  else
    Result := 0; // Default value if not available
end;

function TncLine.SetReceiveTimeout(const Value: Integer): Boolean;
var
  Opt: Cardinal;
  OptSize: Integer;
begin
  ...
  OptSize := SizeOf(Opt);
{$IFDEF MSWINDOWS}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_RCVTIMEO, PAnsiChar(@Opt), OptSize));
{$ELSE}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_RCVTIMEO, Opt, OptSize));
{$ENDIF}
end;
...
begin
  OptSize := SizeOf(Opt);
{$IFDEF MSWINDOWS}
  SocketResult := GetSockOpt(FHandle, SOL_SOCKET, SO_SNDTIMEO, PAnsiChar(@Opt), OptSize);
{$ELSE}
  SocketResult := GetSockOpt(FHandle, SOL_SOCKET, SO_SNDTIMEO, Opt, OptSize);
{$ENDIF}
  if CheckResult(SocketResult) then
    Result := Opt
  else
...
function TncLine.SetSendTimeout(const Value: Integer): Boolean;
var
  Opt: Cardinal;
  OptSize: Integer;
begin
  ...
  OptSize := SizeOf(Opt);
{$IFDEF MSWINDOWS}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_SNDTIMEO, PAnsiChar(@Opt), OptSize));
{$ELSE}
  Result := CheckResult(SetSockOpt(FHandle, SOL_SOCKET, SO_SNDTIMEO, Opt, OptSize));
{$ENDIF}
end;
