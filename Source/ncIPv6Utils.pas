...
type
  ...
  public
    ...
    class function IsValidAddress(const AddrStr: string): Boolean;
...
class function TIPv6AddressUtils.LoadIPv6Functions: Boolean;
begin
  Result := False;
...
class constructor TIPv6AddressUtils.Create;
begin
  {$IFDEF MSWINDOWS}
  if not LoadIPv6Functions then
    raise EIPv6Error.Create('Failed to load IPv6 functions from ws2_32.dll');
  {$ENDIF}
end;

class function TIPv6AddressUtils.StorageToString(const Storage: TSockAddrStorage): string;
begin
...
class function TIPv6AddressUtils.IsIPv6Storage(const Storage: TSockAddrStorage): Boolean;
begin
  Result := Storage.ss_family = AF_INET6;
end;

class function TIPv6AddressUtils.GetStorageFamily(const Storage: TSockAddrStorage): Word;
begin
  Result := Storage.ss_family;
end;

class function TIPv6AddressUtils.StorageToIPv6Address(const Storage: TSockAddrStorage;
  out Addr: TSockAddrIn6): Boolean;
begin
  Result := Storage.ss_family = AF_INET6;
  if Result then
...
class function TIPv6AddressUtils.GetIPFromStorage(const Storage: TSockAddrStorage): string;
begin
  Result := StorageToString(Storage);
end;

class function TIPv6AddressUtils.GetPortFromStorage(const Storage: TSockAddrStorage): Word;
begin
  case Storage.ss_family of
...
class function TIPv6AddressUtils.IsValidAddress(const AddrStr: string): Boolean;
var
  Addr: TIn6Addr;
begin
...
class function TIPv6AddressUtils.AddressToString(const Addr: TIn6Addr): string;
var
...
class function TIPv6AddressUtils.StringToAddress(const AddrStr: string; out Addr: TIn6Addr): Boolean;
var
  AnsiAddr: AnsiString;
begin
...
class function TIPv6AddressUtils.IsLinkLocal(const AddrStr: string): Boolean;
begin
...
class function TIPv6AddressUtils.NormalizeAddress(const AddrStr: string): string;
var
  Addr: TIn6Addr;
begin
  if StringToAddress(AddrStr, Addr) then
    ...
    raise EIPv6Error.CreateFmt('Invalid IPv6 address: %s', [AddrStr]);
end;

class function TIPv6AddressUtils.AddressToPresentation(const Addr: TIn6Addr): string;
var
  i: Integer;
...
class function TIPv6AddressUtils.PresentationToAddress(const Present: string;
